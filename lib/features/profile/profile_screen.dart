...
class ProfileScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final items = [
      _MenuItem(icon: Icons.access_time_rounded, label: 'Geçmiş siparişler', sub: '14 sipariş', go: 'past'),
      _MenuItem(icon: Icons.location_on_outlined, label: 'Kayıtlı mağazalar', sub: 'Bağdat Cd.', go: 'stores'),
                ...
                child: Column(
                  children: items.asMap().entries.map((e) {
                    ...
                    return GestureDetector(
                      onTap: () {
                        if (item.go == 'past') {
                          Navigator.of(context).push(MaterialPageRoute(builder: (_) => const PastOrdersScreen()));
                        } else if (item.go == 'stores') {
                          Navigator.of(context).push(MaterialPageRoute(
                            builder: (_) => StoresScreen(currentStore: KeopiData.stores[0], onPick: (_) => Navigator.pop(context)),
                          ));
                        }
                      },
                      child: Container(
